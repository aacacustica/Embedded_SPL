...
def main():
    try:
        ...
        logging.info(f"location_record raw: {repr(location_record)}")
        logging.info(f"location_place raw: {repr(location_place)}")
        logging.info(f"location_point raw: {repr(location_point)}")
        logging.info(f"storage_output_wav_folder raw: {repr(storage_output_wav_folder)}")
        logging.info(f"audio_path raw: {repr(audio_path)}")

        if not os.path.isdir(audio_path):
            raise ValueError(f"Audio path does not exist: {audio_path}")

        # Carpeta destino de CSVs
        ...
        for audio_file in tqdm.tqdm(valid_audio_files, desc="Processing audio files"):
            try:
                ...
                if file_data:
                    ...
                    logging.warning(f"No usable data produced for {audio_file}; not marking as processed.")
        ...
        flat_data = [row for file_rows in all_data for row in file_rows]

        if not flat_data:
            logging.warning("No rows to save. Skipping CSV.")
            return

        logging.info(f"Total rows to save: {len(flat_data)}")
        logging.info(f"Column names: {col_names}")

        flat_data_sorted = sorted(flat_data, key=lambda row: parse_dt(row[-1]))
        ...
        for audio_file in processed_now:
            update_processed_files(processed_files_txt, audio_file)

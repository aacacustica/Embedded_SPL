...
upload_queue = Queue(maxsize=10)  # Limitar a 10 archivos en cola para evitar saturación 
...
cfg = Config(
        retries={"max_attempts": 10, "mode": "adaptive"}, # [TEST] adaptative so it retries at least once for each part if time out
        connect_timeout=10,
        read_timeout=180, # [TEST] read_timeout 60 -> 180 
        request_checksum_calculation="when_required",
        response_checksum_validation="when_required",
    )


def _put_part(s3, bucket_name, part_key, chunk,max_retries=3, base_sleep=1.0):
...
def upload_worker(
    ...
    while True:
        actual_time = datetime.datetime.now().strftime(strf_time)
        if actual_time.endswith("00:00"):

            logger.info("Uploading control audio to AWS S3 bucket")
            ...
            try:
                ...
                MAX_WORKERS = 4          # [TODO: TEST ]
                IN_FLIGHT = MAX_WORKERS * 2  # cuántas partes “en vuelo” a la vez

                uploaded_parts = 0
                idx = 0
                futures = []

                with open(file_path, "rb") as f, ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
                    while True:
                        ...
                        futures.append(ex.submit(_put_part, s3, bucket_name, part_key, chunk))                    
                        idx += 1

                        # cuando hay muchas en vuelo, espera a que terminen
                        if len(futures) >= IN_FLIGHT:
                            for fut in as_completed(futures):
                                part_uploaded = fut.result()  # si falla, lanza excepción
                                uploaded_parts += 1
                                last_successful_upload_time = time.time()
                                logger.info(f"Uploaded {part_uploaded} ({uploaded_parts}/{parts_count})")
                            futures.clear()

                    # espera las que queden
                    for fut in as_completed(futures):
                        part_uploaded = fut.result()
                        uploaded_parts += 1
                        last_successful_upload_time = time.time()
                        logger.info(f"Uploaded {part_uploaded} ({uploaded_parts}/{parts_count})")
...
def record_segment(stream, p, record_seconds, location_record, location_place, location_point, audio_format, audio_channels, audio_sample_rate, audio_chunk_size, storage_s3_bucket_name, storage_output_wav_folder, logging):
    ...
    # output folder
    output_folder = os.path.join(home_dir, location_record, location_place, location_point,'AUDIOMOTH', storage_output_wav_folder)
    ...
    timestamp_str = datetime.datetime.now().strftime(strf_time_daystamp)
